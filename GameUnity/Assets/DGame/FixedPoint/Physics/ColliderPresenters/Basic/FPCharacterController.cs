...
namespace DGame.FixedPoint
{
    ...
    /// <summary>
    /// 定点角色控制器。纯逻辑部分双端共用；Unity 表现部分（Awake 注册 / OnViewUpdate / OnDrawGizmosEditor）见 FPCharacterController.Unity.cs。
    /// 通过继承自 FPCollider 的 <see cref="FPCollider.context"/> 获取物理上下文，不依赖静态单例。
    /// </summary>
    public sealed partial class FPCharacterController : FPAACapsuleCollider
    {
        ...
        public void Jump(FixedPoint64 jumpHigh)
        {
            ...
            var t = FixedPointMath.Sqrt(FixedPointMath.Abs(jumpHigh * reverseMass / FPPhysicsContext.GravitationalAcceleration.y * 2));
            var v = t * FPPhysicsContext.GravitationalAcceleration.y;
            // 破口 #2 修复：去掉 .AsFloat()，全程 FixedPoint64，保证两端位级一致。
        ...
        public void AddForce()
        {
            ...
            else
            {
                ...
                forces = currentForce + FPPhysicsContext.GravitationalAcceleration - new FixedPointVector3(0, FixedPointMath.Min(MaxAdditionGravity, mass - BaseMass) , 0) ;
            }
