...
namespace DGame.FixedPoint
{
    public sealed class FPRigidbody : FastListItem
    {
        ...
        {
            var count = context.fpOctree.OverlaySphereCollision(transform.position ,collider.radius,ref collisions);
            ...
            count = context.fpOctree.OverlayCharacterWithSphere(this.collider, ref collisions);
            for (var i = 0; i < count; i++)
            {
        ...
        {
            if (context.fpOctree.IsOutOfBound(transform.position))
            {
                return;
            }
