...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0run_unity_with_log.ps1" -UnityEditorPath "%UNITYEDITOR_PATH%" -ProjectPath "%UNITY_PROJECT_PATH%" -LogFile "%BUILD_LOGFILE%" -ExecuteMethod "DGame.ReleaseTools.BuildAndroidABWithVersion" -Version "%VERSION%"
